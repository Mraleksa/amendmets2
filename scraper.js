...
//console.log(changeLength+"-"+upDate);
	...
	statement.run(item.dateModified,dateSigned,first,data.getJSON().data.tenderID,data.getJSON().data.procuringEntity.name,data.getJSON().data.numberOfBids,startAmount,amount,data.getJSON().data.items[0].classification.description,region,up,down,upDate,downDate);
	//console.log(change);
	statement.finalize();
	});
	}		
